 ...
 * $m= new Mail;
   ...
   $m->Subject('Hi100|From FeedBack form');
   $m->Body($this->post['body']);
   $m->Priority(3);
   $m->Cc("copy@asd.com");
   $m->Bcc("bcopy@asd.com");
...
class ComMail
{
	...
	function Send()
	{
		...
		else
		{
			...
			$this->smtp_log .= $data = $this->get_data($smtp_conn)."\n";
			$code = substr($data,0,3);
			if($code != 250) {$this->smtp_log .= "Code != 250 MAIL FROM\n"; fclose($smtp_conn); return  false;}
			foreach ($this->smtpsendto as $keywebi => $valuewebi)
			{
				...
				$this->smtp_log .= $data = $this->get_data($smtp_conn)."\n";
				$code = substr($data,0,3);
				if($code != 250 AND $code != 251) {$this->smtp_log .= "Code != 250 RCPT TO\n"; fclose($smtp_conn); return  false;}
			}
			fputs($smtp_conn,"DATA\r\n");
			$this->smtp_log .="ß: DATA\n";
			$this->smtp_log .= $data = $this->get_data($smtp_conn)."\n";
			$code = substr($data,0,3);
			if($code != 354) {$this->smtp_log .= "Code != 354 DATA\n"; fclose($smtp_conn); return  false;}
			fputs($smtp_conn,$this->headers."\r\n".$this->fullBody."\r\n.\r\n");
			$this->smtp_log .= "ß: ".$this->headers."\r\n".$this->fullBody."\r\n.\r\n";
			$this->smtp_log .= $data = $this->get_data($smtp_conn)."\n";
			$code = substr($data,0,3);
			if($code != 250) {$this->smtp_log .= "Code != 250\n"; fclose($smtp_conn); return  false;}
		...
		for( $i=0; $i < count( $this->aattach); $i++ )
                {
                    ...
                    if(!file_exists($filename))
                        {
                            echo "Not found file ".$filename; exit;
			}
