...
/**
 * Description of cryptClass
 *
 * @author ishibkikh
 */
abstract class SysCrypt {
     ...
     */
    protected static function Cipher($input, $w) {    // main Cipher function [Â§5.1]
    ...
    protected static function AddRoundKey($state, $w, $rnd, $Nb) {  // xor Round Key into state S [Â§5.1.4]
        for ($r=0; $r<4; $r++) {
    ...
    protected static function SubBytes($s, $Nb) {    // apply SBox to state S [Â§5.1.1]
        for ($r=0; $r<4; $r++) {
     ...
     */
    protected static function KeyExpansion($key) {  // generate Key Schedule from Cipher Key [Â§5.2]
        $Nb = 4;              // block size (in words): no of columns in state (fixed at 4 for AES)
